 ...
 *
 *  Column Q — "Show on website":
 *    Every parsed row carries a `showOnWebsite` boolean. The website build
 *    (src/_data/curriculumData.js) drops any row set to false. The PDFs
 *    ignore the flag by default — see PDF_RESPECTS_SHOW_ON_WEBSITE below.
 */
...
var COL = {
  ...
  COMPULSORY: 15, SHOW_ON_WEBSITE: 16
};

/* Resolved from the header row on every parse, so moving the "Show on website"
   column doesn't break anything. Falls back to COL.SHOW_ON_WEBSITE (Q) when the
   header text isn't recognised; -1 only if that column is missing entirely. */
...
/* Set to true to make the PDFs skip rows hidden from the website too.
   Left false so the PDFs stay the complete internal document. */
var PDF_RESPECTS_SHOW_ON_WEBSITE = false;
 ...
 *
 * Note: an *unticked checkbox* exports the boolean false, which hides the row.
 * If you use checkboxes you must tick every row you want published. Typing
 * FALSE only in the rows to hide is the lower-effort option.
 */
...
/* ── PDF-side visibility filters (no-ops unless PDF_RESPECTS_SHOW_ON_WEBSITE) ── */

function pdfLevels(curriculum) {
  if (!PDF_RESPECTS_SHOW_ON_WEBSITE) return curriculum.levels;
  return curriculum.levels.filter(function(lv) { return lv.showOnWebsite !== false; });
}

function pdfSections(lv) {
  if (!PDF_RESPECTS_SHOW_ON_WEBSITE) return lv.sections;
  return lv.sections.filter(function(sec) { return sec.showOnWebsite !== false; });
}

function pdfItems(sec) {
  if (!PDF_RESPECTS_SHOW_ON_WEBSITE) return sec.items;
  return sec.items.filter(function(it) { return it.showOnWebsite !== false; });
}
    ...
    switch (type) {
      case 'levelHeader':
        ...
        currentLevel = {
          ...
          sections: [], notes: [], showOnWebsite: readShow(row)
        };
        result.levels.push(currentLevel);
        currentSection = null; currentModule = null;
        break;
      ...
      case 'sectionHeader':
        ...
        currentSection = {
          title: str(row[COL.TOPIC]), totalHours: toNum(row[COL.HOURS]),
          items: [], showOnWebsite: readShow(row)
        };
...
function generateCurriculumPDF() {
  ...
  var levels = pdfLevels(curriculum);
  ...
  levels.forEach(function(lv) {
    ...
    pdfSections(lv).forEach(function(sec) {
      pdfItems(sec).forEach(function(it) {
        if (it.type === 'module') {
  ...
  // ── Level details ──
  levels.forEach(function(lv, lvIdx) {
    ...
    pdfSections(lv).forEach(function(sec) {
      ...
      pdfItems(sec).forEach(function(it) {
        if (it.type === 'module') {
          ...
          // Sub-modules
          var subModules = it.subModules || [];
          if (PDF_RESPECTS_SHOW_ON_WEBSITE) {
            subModules = subModules.filter(function(sm) { return sm.showOnWebsite !== false; });
          }
    ...
    var notes = lv.notes || [];
    if (PDF_RESPECTS_SHOW_ON_WEBSITE) {
      notes = notes.filter(function(n) { return n.showOnWebsite !== false; });
    }
    if (notes.length) {
...
function generateProgramPDF() {
  ...
  for (var r = 1; r < table.getNumRows(); r++) {
    ...
    row.getCell(0).getChild(0).asText().setBold(true).setForegroundColor(
      STYLE.LEVEL_COLORS[levels[r - 1].level] || STYLE.PRIMARY
    );
  }
  ...
  levels.forEach(function(lv) {
    ...
    // Notes
    var lvNotes = lv.notes || [];
    if (PDF_RESPECTS_SHOW_ON_WEBSITE) {
      lvNotes = lvNotes.filter(function(n) { return n.showOnWebsite !== false; });
    }
    if (lvNotes.length) {
      lvNotes.forEach(function(n) {
