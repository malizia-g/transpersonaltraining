...
| **`old.` subdomain** | Never indexed. Verified 2026-09-08 with URL Inspection on both the homepage and `/jure-biechoniski/` — the highest-traffic bio, and the URL the whole worry was about: both report **"URL unknown to Google"**. It was reachable and indexable for a while, but the only link to it sits in the homepage modal, and that homepage only became crawlable at the 7 Sep cutover, so Google never got there. `noindex` is now set on the host (verified: header present on pages, bios and uploads, with `robots.txt` left open so the directive can be read), which makes this preventive rather than corrective — no GSC removal request needed. **Correction worth keeping:** this was described during the migration as "actively competing" for the 64% of traffic on teacher names. That was inference from it being *indexable*, not a measurement — the index was never checked. The post-cutover numbers are therefore not contaminated by it, which matters for [task 32](#task-32). |
