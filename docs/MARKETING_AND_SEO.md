...
| 4 — Real form backend | ✅ **Built and live.** Google Apps Script web app → contact messages and applications land in a spreadsheet, signed agreements in Drive, notification email to the office. Honeypot, GDPR consent text and on-page success/error feedback all present. No `mailto:` action any more (it made Chrome flag the form as not secure); without JS a `<noscript>` note gives the office addresses. See [APPLICATION_PAGE_SETUP.md](APPLICATION_PAGE_SETUP.md) |
...
| 4 | ✅ **DONE (Jul 2026)** — real form backend live (Apps Script → Sheets + Drive + notification email). `mailto:` action removed, `<noscript>` note instead | A | S | — |
...
The form has no `mailto:` action any more — Chrome called it "not secure" and switched off autofill; without JavaScript a `<noscript>` note shows the office addresses.
